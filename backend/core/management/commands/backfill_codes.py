...
TARGETS = [
    ("ingredients", "Ingredient", "ING", "code", 4),
    ...
    ("orders", "Order", "SOT", "order_number", 6),
]
            ...
            for obj in qs.iterator():
                ...
                try:
                    ...
                    while attempts < 5:
                        attempts += 1
                        try:
                            # For orders, compute year-prefixed SOT prefix per-object
                            real_prefix = prefix
                            if app_label == "orders" and model_name == "Order" and field == "order_number":
                                created = getattr(obj, "created_at", None)
                                year = getattr(created, "year", None) if created else None
                                if year:
                                    real_prefix = f"SOT-{year}"

                            code = generate_and_set_code(obj, real_prefix, field, width)
                            if dry_run:
